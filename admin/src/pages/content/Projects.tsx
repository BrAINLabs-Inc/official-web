...
import { Plus, Pencil, Trash2, Star, X, ChevronDown, List } from "lucide-react";
...
export default function Projects() {
  ...
  useEffect(() => { 
    if (t) {
      api.projects.list(t).then((d) => { setProjects(d); setLoading(false); }).catch(e => {
                ...
                <tr key={p.id} className="hover:bg-zinc-50 transition-colors">
                  ...
                  <td className="px-5 py-4">
                    <div className="flex items-center justify-end gap-1">
                      {isAdmin && (
                        <button onClick={() => handleTogglePublish(p)} className="p-1.5 rounded-lg text-zinc-400 hover:text-zinc-900 hover:bg-zinc-100 transition-colors">
                          <Star size={14} fill={p.status === "PUBLISHED" ? "currentColor" : "none"} />
                        </button>
                      )}
                      <button onClick={() => setEditingProject(p)} className="p-1.5 rounded-lg text-zinc-400 hover:text-zinc-900 hover:bg-zinc-100 transition-colors"><Pencil size={14} /></button>
                      <button onClick={() => handleDeleteProject(p.id!)} className="p-1.5 rounded-lg text-zinc-400 hover:text-red-600 hover:bg-red-50 transition-colors"><Trash2 size={14} /></button>
                    </div>
                  </td>
                </tr>
              ))}
            ...
            <div className="flex-1 overflow-y-auto w-full flex flex-col">
              <div className="p-6 border-b border-zinc-100 bg-zinc-50/50">
                <Field label="Project Category or Name">
                  <input className="field-input w-full" value={editingProject.category ?? ""} onChange={(e) => setEditingProject((prev) => ({ ...prev, category: e.target.value }))} placeholder="e.g. Current Research" />
                </Field>
                ...
                <Field label="Description">
                  <textarea className="field-input w-full min-h-[60px]" value={editingProject.description ?? ""} onChange={(e) => setEditingProject((prev) => ({ ...prev, description: e.target.value }))} placeholder="High-level description of this project bucket..." />
                </Field>
