...
import { Plus, Pencil, Trash2, X, BookOpen, FileText, ChevronDown } from "lucide-react";
...
export default function Tutorials() {
  ...
  useEffect(() => { 
    if (t) {
      api.tutorialSeries.list(t).then((d) => { setSeries(d); setLoading(false); }).catch(e => {
    ...
    <div className="p-6 lg:p-8 max-w-6xl mx-auto flex flex-col h-full">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-0.5">Create structured educational deep-dives.</p>
        </div>
          ...
          <div className="relative ml-auto w-full max-w-4xl bg-white h-full shadow-xl flex flex-col md:flex-row animate-in slide-in-from-right duration-200">
            ...
            <div className="w-full md:w-80 border-r border-zinc-100 flex flex-col bg-zinc-50/30">
               <div className="p-6 border-b border-zinc-100">
                  ...
                  <Field label="Title">
                     <input className="field-input w-full bg-white text-xs py-2" value={editingSeries.title} onChange={e => setEditingSeries((p: any) => ({ ...p, title: e.target.value }))} />
                  </Field>
                  <Field label="URL Slug">
                     <input className="field-input w-full bg-white text-xs py-2" value={editingSeries.slug || ""} onChange={e => setEditingSeries((p: any) => ({ ...p, slug: e.target.value.toLowerCase().replace(/ /g, '-') }))} />
                  </Field>
                  <Field label="Status">
                  ...
                  <div className="flex justify-end gap-2 mt-4">
                     <button onClick={handleSaveSeries} className="px-3 py-1.5 bg-black text-white text-[10px] font-bold uppercase tracking-widest rounded-lg">{saving ? '...' : 'Save'}</button>
                  </div>
               </div>
               ...
               {editingPage ? (
                 <>
                    <div className="p-6 border-b border-zinc-100 flex items-center justify-between bg-white">
                       <div>
                         <h3 className="text-sm font-bold text-zinc-900">{editingPage.id ? 'Edit Page' : 'New Page'}</h3>
                         <p className="text-[10px] text-zinc-500 uppercase tracking-widest">Page in {editingSeries.title}</p>
                       </div>
                       <div className="flex gap-2">
                         <button onClick={() => setEditingPage(null)} className="px-4 py-1.5 text-xs text-zinc-500 hover:text-zinc-900">Cancel</button>
                    ...
                    <div className="p-8 flex-1 overflow-y-auto bg-white">
                       <div className="grid grid-cols-2 gap-6 mb-6">
                         <Field label="Page Title">
                            <input className="field-input w-full bg-zinc-50" value={editingPage.title || ""} onChange={e => setEditingPage((p: any) => ({ ...p, title: e.target.value }))} placeholder="Introduction" />
                         </Field>
                         <Field label="URL Slug">
                            <input className="field-input w-full bg-zinc-50" value={editingPage.slug || ""} onChange={e => setEditingPage((p: any) => ({ ...p, slug: e.target.value.toLowerCase().replace(/ /g, '-') }))} placeholder="intro" />
                         </Field>
                       </div>
                       <Field label="Content (Markdown)">
                          <textarea className="field-input w-full min-h-[400px] bg-zinc-50 font-mono text-xs leading-relaxed p-6" value={editingPage.content || ""} onChange={e => setEditingPage((p: any) => ({ ...p, content: e.target.value }))} placeholder="# Document your research..." />
                       </Field>
                    </div>
                 </>
               ) : (
